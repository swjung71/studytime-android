...
/**
 * Created by Seung Wook Jung on 2015-08-04.
 */
public class MD5ForAdultURL {

    public static void main(String[] args) {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        try{
            while(true) {
                System.out.println("Enter string('exit' to quit)");
                String line = br.readLine();
                if ( line.equals("exit") ) break;
                System.out.println(new MD5ForAdultURL().toDigest(line));
            }
        }catch(IOException e) {}
    }

    byte m_digest[] = new byte[16];

    public synchronized void Encoding(String msg,byte[] result) {
        System.arraycopy(toDigest(msg).getBytes(),0,result,0,result.length);
    }

    public synchronized boolean equals(String msg,byte[] encoded) {
        byte tmp[] = new byte[32];
        Encoding(msg,tmp);

        try{
            for( int i=0; i<tmp.length; i++ )
                if ( tmp[i] != encoded[i] ) return false;
        }catch(Exception e) {
            return false;
        }
